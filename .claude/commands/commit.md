...
allowed-tools: Bash(git:*), Bash(mvn:*), Bash(npm:*), AskUserQuestion
---
...
Before committing, run the following quality checks:

1. **Format and compile backend** (if Java files changed):
   ```bash
   cd /Users/roelandhofkens/Projects/Ai/video-story-creator/backend && mvn spotless:apply compile
   ```

2. **Lint frontend** (if frontend files changed):
   ```bash
...
The project uses **conventional commits** with a pre-commit hook that validates the format:
...
4. Run quality checks (spotless, lint) as needed
